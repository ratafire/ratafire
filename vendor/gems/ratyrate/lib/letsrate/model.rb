...
module Letsrate
  ...
  def can_rate?(user_id, dimension=nil)
    val = self.connection.select_value("select count(*) as cnt from rates where rateable_id=#{self.id} and klass='#{self.class.name}' and rater_id=#{user_id} and dimension='#{dimension}'").to_i
    if val == 0
      true
    else
      false
