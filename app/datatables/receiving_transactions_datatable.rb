class ReceivingTransactionsDatatable
  ...
  def as_json(options = {})
    {
      ...
      iTotalRecords: Transaction.where(:subscribed_id => @user.id).count,
...
private
  ...
  def data
    transactions.map do |transaction|
      [
        ...
        link_to(truncate(subscriber_fullname(transaction.subscriber_id), :length => 30),user_path(transaction.subscriber_id), class:"no_ajaxify"),
        transaction.status,
        "$"+transaction.amount,
  ...
  def fetch_transactions
    transactions = Transaction.where(:subscribed_id => @user.id).order("#{sort_column} #{sort_direction}")
