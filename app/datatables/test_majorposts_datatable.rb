class TestMajorpostsDatatable
  delegate :params, :h, :link_to, :user_project_majorpost_path, :untest_path, :truncate, to: :@views
...
private
  ...
  def data
    majorposts.map do |majorpost|
      [
        ...
        link_to("Untest", untest_path("Majorpost",majorpost.id), class:"no_ajaxify")
      ]
    end
  end
  ...
  def fetch_majorposts
    majorposts = Majorpost.where(:test => true).order("#{sort_column} #{sort_direction}")
