class TestProjectsDatatable
  delegate :params, :h, :link_to, :user_project_path, :untest_path, :truncate, to: :@view
...
private
  ...
  def data
    projects.map do |project|
      [
        ...
        link_to("Untest", untest_path("Project",project.id), class:"no_ajaxify")
      ]
    end
  end
  ...
  def fetch_projects
    projects = Project.where(:test => true).order("#{sort_column} #{sort_direction}")
