 	...
 	def self.create_transfer_and_order(subscription_id)
		...
		if @subscribed != nil && subscription.created_at < ( Time.now - 7.days ) then
			...
			if @subscribed.facebookupdates.first != nil then
				if @subscribed.facebookupdates.first.created_at > ( Time.now - 31.days ) then
					...
					if @subscribed.majorposts.first != nil then
						if @subscribed.majorposts.first.created_at > ( Time.now - 31.days ) then
							#Create or find a transfer
							unless subscription.method == "Venmo" then
								if @subscribed.transfer != nil then
									@transfer = @subscribed.transfer
							...
							if @subscribed.projects.where("published_at IS NOT NULL").first != nil then
								if @subscribed.projects.where("published_at IS NOT NULL").first.published_at > ( Time.now - 31.days ) then
									#Create or find a transfer
									unless subscription.method == "Venmo" then
										if @subscribed.transfer != nil then
											@transfer = @subscribed.transfer
									 ...
									 if @subscribed.discussions.where(:review_status => "Approved").first != nil then
									 	if @subscribed.discussions.where(:review_status => "Approved").first.created_at > ( Time.now - 31.days ) then
											#Create or find a transfer
											unless subscription.method == "Venmo" then
												if @subscribed.transfer != nil then
													@transfer = @subscribed.transfer
