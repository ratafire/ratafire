class AudioImage < ActiveRecord::Base
    ...
    has_attached_file :image, 
        ...
        :default_url => lambda { |av| "/assets/default/audio/audio#{av.instance.default_image_number}.jpg" },
        :convert_options => { :all => '-background "#c8c8c8" -flatten +matte'},
        :url =>  "/:class/uploads/:id/image/:style/:uuid_audio_image_filename",
        #If s3
        :path => "/:class/uploads/:id/image/:style/:uuid_audio_image_filename",
    ...
    Paperclip.interpolates :uuid_audio_image_filename do |attachment, style|
        attachment.instance.audio_image_uuid_to_filename
    end

    def audio_image_uuid_to_filename
        "#{self.uuid}"
    end
...
private
    ...
    def generate_uuid!
        begin
            self.uuid = SecureRandom.hex(14)
        end while AudioImage.find_by_uuid(self.uuid).present?
    end
